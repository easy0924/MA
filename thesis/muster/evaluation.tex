...
The reordering rules were extracted by using the alignment, POS tags and syntax trees from the training data. One reference of the test data was used for evaluating the \ac{BLEU} scores. The threshold for rule extraction is set as $5$ times and reordering paths with probability less than $0.1$ are not added to the lattice. The baseline used a monotone decoder and no reordering at all. Table~\ref{denw} shows the size of data used in the system. %? case-sensitive?
